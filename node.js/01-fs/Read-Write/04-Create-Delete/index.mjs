...
export const createFileWithMessage = async (message) => {
    try {
        ...
        const fileName = `${now.getHours().toString().padStart(2, '0')}-${now.getMinutes().toString().padStart(2, '0')}-${now.getSeconds().toString().padStart(2, '0')}.txt`;
...
if (command === 'create' && argument) {
    ...
    console.log('Invalid command or missing argument. Use "create <message>" or "delete <fileName>".');
}